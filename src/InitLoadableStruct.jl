...
load_data(t::Type{TYPE}, args...; kw_args...) where TYPE <: InitableLoadable = load_data!(init(t, args...; kw_args...))
load_data!(obj::TYPE)                         where TYPE <: InitableLoadable = throw("Implement load_data(...)")

unique_args(obj::TYPE)                        where TYPE <: InitableLoadable = throw("We need the fields that specify the data for caching")
